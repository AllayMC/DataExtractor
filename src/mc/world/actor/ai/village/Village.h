...
public:
    // NOLINTBEGIN
    inline auto& $ACHIEVEMENT_SOUND_THE_ALARM_TOLERANCE() { return ACHIEVEMENT_SOUND_THE_ALARM_TOLERANCE; }

    inline auto& $CELEBRATION_EVENT() { return CELEBRATION_EVENT; }

    inline auto& $RAID_BOUNDS_PADDING() { return RAID_BOUNDS_PADDING; }

    inline auto& $RAID_EXPIRY_EVENT() { return RAID_EXPIRY_EVENT; }

    inline auto& $RAID_FINISHED_DELAY_IN_TICKS() { return RAID_FINISHED_DELAY_IN_TICKS; }

    inline auto& $RAID_GROUP_COMPLETE_DELAY_IN_TICKS() { return RAID_GROUP_COMPLETE_DELAY_IN_TICKS; }

    inline auto& $RAID_LOCATION_HELP_DELAY_IN_TICKS() { return RAID_LOCATION_HELP_DELAY_IN_TICKS; }

    inline auto& $RAID_MAX_SPAWN_FAILURES() { return RAID_MAX_SPAWN_FAILURES; }

    inline auto& $RAID_PREP_TIME_IN_TICKS() { return RAID_PREP_TIME_IN_TICKS; }

    inline auto& $RAID_TIMEOUT_IN_TICKS() { return RAID_TIMEOUT_IN_TICKS; }

    inline auto& $STANDING_DECAY_INTERVAL_BAD() { return STANDING_DECAY_INTERVAL_BAD; }

    inline auto& $STANDING_DECAY_INTERVAL_GOOD() { return STANDING_DECAY_INTERVAL_GOOD; }

    inline auto& $STORAGE_KEY_DWELLERS() { return STORAGE_KEY_DWELLERS; }

    inline auto& $STORAGE_KEY_PLAYERS() { return STORAGE_KEY_PLAYERS; }

    inline auto& $STORAGE_KEY_POI() { return STORAGE_KEY_POI; }

    inline auto& $STORAGE_KEY_RAID() { return STORAGE_KEY_RAID; }

    inline auto& $STORAGE_KEY_VILLAGE() { return STORAGE_KEY_VILLAGE; }

    inline auto& $UNINITIALIZED_VILLAGE_LIFETIME_MAX_TICKS() { return UNINITIALIZED_VILLAGE_LIFETIME_MAX_TICKS; }

    inline auto& $VILLAGER_EXPECTED_WORK_INTERVAL() { return VILLAGER_EXPECTED_WORK_INTERVAL; }

    inline auto& $VILLAGE_HERO_BESTOW_TIME() { return VILLAGE_HERO_BESTOW_TIME; }

    inline auto& $VILLAGE_POI_PURGE_TIME() { return VILLAGE_POI_PURGE_TIME; }

    inline auto& $VILLAGE_RADIUS_APPROX_SCALAR() { return VILLAGE_RADIUS_APPROX_SCALAR; }

    inline auto& $VILLAGE_SAVE_TICK_DELAY() { return VILLAGE_SAVE_TICK_DELAY; }

    inline auto& $VILLAGE_START_HEIGHT() { return VILLAGE_START_HEIGHT; }

    inline auto& $VILLAGE_UNCLAIMED_POI_CAP() { return VILLAGE_UNCLAIMED_POI_CAP; }

    inline auto& $VILLAGE_UPDATE_TICK_DELAY() { return VILLAGE_UPDATE_TICK_DELAY; }

    inline auto& $VILLAGE_VERSION_CURRENT() { return VILLAGE_VERSION_CURRENT; }
