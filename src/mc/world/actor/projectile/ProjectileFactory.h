...
public:
    ...
    MCAPI class Actor * shootProjectileFromDefinition(struct ActorDefinitionIdentifier const &, class Mob *, class Vec3, int32_t, float);
...
private:
    ...
    MCAPI static std::unordered_map<std::string,std::function<std::unique_ptr<class OnHitSubcomponent> (void)>> mSubcomponentMap;
...
public:
    // NOLINTBEGIN
    inline auto& $mSubcomponentMap() { return mSubcomponentMap; }
