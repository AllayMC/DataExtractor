...
public:
    // NOLINTBEGIN
    // symbol: ?createResetMoveDirectionJumpPendingSystem@ResetMoveDirectionJumpPendingSystem@@SA?AUTickingSystemWithInfo@@XZ
    ...
    MCAPI static void tickResetMoveDirectionJumpPendingSystem(class ViewT<class StrictEntityContext, class EntityRegistryBase, struct Include<class FlagComponent<struct OnGroundFlag>>, struct Exclude<class FlagComponent<struct MobAllowStandSlidingFlag>>, struct JumpPendingScaleComponent const, struct SynchedActorDataComponent const, struct VehicleComponent const, struct MobTravelComponent>);
