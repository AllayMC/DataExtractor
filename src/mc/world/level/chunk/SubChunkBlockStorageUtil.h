...
namespace SubChunkBlockStorageUtil {
    ...
    MCAPI std::unique_ptr<class SubChunkStorage<class Block>> makeDeserialized(class IDataInput &, class BlockPalette const &, std::function<class Block const * (uint64)> const &, std::function<class Block const * (class CompoundTag const &)> const &, ::SubChunkFormat);
    // NOLINTEND
