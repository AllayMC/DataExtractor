...
public:
    ...
    MCAPI DBStorageEnvironmentChain(struct DBStorageConfig const &, class Core::Path const &, gsl::not_null<class Bedrock::NonOwnerPointer<class LevelDbEnv>>);
    ...
    MCAPI static bool isContentKeyValid(leveldb::Env *, class Core::Path const &, class ContentIdentity const &, std::string const &, std::shared_ptr<class Core::FileStorageArea>);
    ...
    MCAPI static std::unique_ptr<class FlushableEnv> createFlushableEnv(leveldb::Env *, std::shared_ptr<class Core::FileStorageArea>, class Core::Path const &);
